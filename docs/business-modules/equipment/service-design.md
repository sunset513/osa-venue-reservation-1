...
9. 核准成功後，查詢其他同日期、時段重疊、設備有交集的 `pending(1)` 申請。
10. 逐筆重新執行 availability 檢查；若該 pending 申請因設備數量不足而不可核准，將其自動改為 `rejected(3)`。

核准時需防止併發超借。多設備申請建議依 `equipment_id ASC` 固定順序鎖定或查詢，避免死鎖。
自動拒絕不可只依時段重疊判斷，因為設備有數量；仍可核准的 pending 申請必須保留。
...
- 若目標為 `2=已通過` 且更新成功，會接續執行「自動拒絕因設備數量不足而不可核准 pending 申請」流程。

## 自動拒絕衝突 pending 設備申請

設備審核核准一筆申請後，系統會檢查其他可能受影響的 `pending(1)` 申請：

1. 查詢候選申請：同一天、時段重疊、設備項目有交集、排除剛核准的申請。
2. 對每筆候選申請用目前已核准設備數量重新執行 availability 檢查。
3. 若候選申請因設備數量不足而不可核准，將該申請改為 `rejected(3)`，寫入 `reviewed_by`、`reviewed_at` 並遞增 `version`。
4. 若候選申請仍有足夠數量可核准，保持 `pending(1)`。

注意：從 `approved(2)` 改回 `rejected(3)` 時，不會自動恢復先前因容量不足被拒絕的 pending 申請。
