...
| `selectApprovedBookingsForThreeVenues` | `venueIdA`, `venueIdB`, `venueIdC`, `date` | `List<ApprovedBookingQueryVO>` | 查詢指定日期三場地已通過預約（含場地名稱與時段遮罩）。 |
...
### 6. 公開查詢三場地已通過預約
```xml
<select id="selectApprovedBookingsForThreeVenues" resultMap="approvedBookingQueryMap">
    SELECT
        b.id AS booking_id,
        b.venue_id,
    ...
    WHERE b.venue_id IN (#{venueIdA}, #{venueIdB}, #{venueIdC})
      AND b.booking_date = #{date}
      AND b.status = 2
    ORDER BY b.venue_id ASC, b.created_at ASC
</select>
```
* **技術說明**：只篩選 `status = 2`（已通過），並一次查出三個場地。
