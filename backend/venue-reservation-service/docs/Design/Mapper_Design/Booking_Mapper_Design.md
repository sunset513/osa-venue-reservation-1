...
| `selectApprovedBookingsForTwoVenues` | `venueIdA`, `venueIdB`, `date` | `List<ApprovedBookingQueryVO>` | 查詢指定日期兩場地已通過預約（含場地名稱與時段遮罩）。 |
...
### 6. 公開查詢兩場地已通過預約
```xml
<select id="selectApprovedBookingsForTwoVenues" resultMap="approvedBookingQueryMap">
    SELECT
        b.id AS booking_id,
        b.venue_id,
...
```
* **技術說明**：只篩選 `status = 2`（已通過），並一次查出兩個場地。
